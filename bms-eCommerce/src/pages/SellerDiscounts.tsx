...
import CreateDiscountModal from "../components/landing/CreateDiscountModal";

type DStatus = "active" | "scheduled" | "disabled" | "expired";
const STATUS: Record<DStatus, { label: string; bg: string; text: string }> = {
...
const TABS: { key: "all" | DStatus; label: string; count: number }[] = [
  { key: "all", label: "โค้ดส่วนลดทั้งหมด", count: 6 },
...
type Row = { id: string; code: string; discount: string; sub?: string; condition: string; start: string; end: string; status: DStatus };
const CODES: Row[] = [
  { id: "c1", code: "BMSECM20", discount: "฿ 80", condition: "ซื้อขั้นต่ำ ฿ 1,000", start: "10/04/2026-00.00", end: "10/05/2026-11.59", status: "active" },
  { id: "c2", code: "BMSECM20", discount: "10 %", condition: "ซื้อขั้นต่ำ ฿ 100", start: "10/04/2026-00.00", end: "10/05/2026-11.59", status: "active" },
  { id: "c3", code: "BMSECM20", discount: "10 %", sub: "สูงสุด ฿ 300", condition: "ซื้อขั้นต่ำ ฿ 500", start: "10/04/2026-00.00", end: "10/05/2026-11.59", status: "active" },
  { id: "c4", code: "BMSECM20", discount: "ส่งฟรี", condition: "สั่งซื้อครั้งแรก", start: "10/04/2026-00.00", end: "10/05/2026-11.59", status: "scheduled" },
  { id: "c5", code: "BMSECM20", discount: "15 %", condition: "ซื้อขั้นต่ำ ฿ 5,000", start: "10/04/2026-00.00", end: "10/05/2026-11.59", status: "disabled" },
  { id: "c6", code: "BMSECM20", discount: "฿ 100", condition: "ซื้อขั้นต่ำ ฿ 500", start: "10/04/2026-00.00", end: "10/05/2026-11.59", status: "expired" },
];

const COLS = "grid-cols-[1.4fr_1.2fr_1.4fr_1.6fr_1fr_1.1fr]";

export default function SellerDiscounts() {
  const [tab, setTab] = useState<(typeof TABS)[number]["key"]>("all");
  const [page, setPage] = useState(2);
  const [createOpen, setCreateOpen] = useState(false);
  const [statuses, setStatuses] = useState<Record<string, DStatus>>(() => Object.fromEntries(CODES.map((c) => [c.id, c.status])));
  ...
  return (
    <div className="min-h-screen bg-[#f5f8fa]">
        ...
        <main className="flex-1 min-w-0 px-8 py-6 flex flex-col gap-4 min-h-[calc(100vh-72px)]">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center">
                <input type="text" placeholder="ค้นหาโค้ดส่วนลด" className="h-10 w-[360px] bg-white border border-[var(--color-neutral-300)] rounded-l-lg px-4 text-[16px] text-[var(--color-neutral-900)] placeholder:text-[var(--color-neutral-500)] focus:outline-none focus:border-[var(--color-primary)]" />
              ...
              <button type="button" onClick={() => setCreateOpen(true)} className="h-10 px-4 rounded-lg bg-[var(--color-primary)] text-white text-[14px] font-medium flex items-center gap-2 hover:bg-[var(--color-primary-600)] transition-colors shrink-0"><Plus size={16} />สร้างโค้ดส่วนลด</button>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl overflow-hidden shadow-[0_2px_4px_rgba(29,33,45,0.08),0_0_2px_rgba(29,33,45,0.08),0_0_1px_rgba(29,33,45,0.2)] flex flex-col flex-1 min-h-0">
            <div className={`grid ${COLS} bg-[var(--color-primary-100)] border-b border-[var(--color-neutral-200)] text-[12px] font-medium uppercase text-[var(--color-neutral-600)]`}>
              <div className="flex items-center h-9 px-4">โค้ดส่วนลด</div>
              <div className="flex items-center h-9 px-2">ส่วนลด</div>
              <div className="flex items-center h-9 px-2">เงื่อนไข</div>
            ...
            <div className="flex-1 min-h-0 overflow-y-auto">
              {rows.map((r) => (
                <div key={r.id} className={`grid ${COLS} items-center border-b border-[var(--color-neutral-200)] last:border-b-0 py-3 hover:bg-[var(--color-primary-100)]/40 transition-colors`}>
                  <div className="flex items-center gap-2 px-4 text-[14px] text-[var(--color-primary)] font-medium">{r.code}<CopyButton value={r.code} /></div>
                  <div className="px-2"><p className="text-[14px] text-[var(--color-neutral-900)]">{r.discount}</p>{r.sub && <p className="text-[12px] text-[var(--color-neutral-500)]">{r.sub}</p>}</div>
                  <div className="text-[14px] text-[var(--color-neutral-900)] px-2 truncate">{r.condition}</div>
                  <div className="px-2 text-[12px] text-[var(--color-neutral-600)] leading-5"><p>เริ่มต้น : {r.start}</p><p>สิ้นสุด : {r.end}</p></div>
                  ...
                  <div className="flex items-center justify-center gap-2 px-2">
                    <Switch size="sm" color="success" isSelected={statuses[r.id] === "active"} onValueChange={(on) => toggle(r, on)} aria-label="เปิด/ปิดโค้ด" />
              ...
              {rows.length === 0 && <div className="py-16 text-center text-[14px] text-[var(--color-neutral-500)]">ไม่มีโค้ดส่วนลด</div>}
            </div>
      ...
      <CreateDiscountModal isOpen={createOpen} onClose={() => setCreateOpen(false)} />
    </div>
  );
}
