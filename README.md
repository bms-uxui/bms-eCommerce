...
- สินค้า — การจัดการสินค้า (`/seller/products`: ตาราง + แท็บสถานะ + สวิตช์เปิด/ปิดขาย ปิดอัตโนมัติเมื่อสินค้าหมด + tooltip) · ฟอร์มเพิ่ม/แก้ไขสินค้า (`/seller/products/new`: stepper 4 ขั้น, ตัวเลือกสินค้าลากจัดลำดับได้ด้วย framer-motion `Reorder`, ตาราง variant แบบ cartesian + ช่องอัปโหลดรูป, การ์ด “ตั้งค่าสินค้า” gradient)
- คำขอคืนเงิน/คืนสินค้า — รายการ (`/seller/refunds`) · รายละเอียด (`/seller/refunds/:id`: ปุ่มอนุมัติ/ปฏิเสธ + ยืนยันรับคืน, timeline ปรับตามประเภท/ผลลัพธ์, รูป/วิดีโอกดขยายเป็น lightbox)
- การตลาด — โปรโมชั่น (`/seller/promotions`) · โค้ดส่วนลด (`/seller/discounts`) · Flash Sale (`/seller/flashsale`: การ์ด event เลื่อนแนวนอน + ตาราง) · รายละเอียด Flash Sale (`/seller/flashsale/:id`); modal สร้างโปรโมชั่น/โค้ดส่วนลด/เพิ่มสินค้า Flash Sale + popup เงื่อนไข Flash Sale
- โลจิสติกส์ (`/seller/logistics`) — ที่อยู่คลังสินค้า (+ modal เพิ่มที่อยู่) · ตั้งค่าการจัดส่ง (โหมดแก้ไข, ยอดส่งฟรี / ค่าจัดส่งเริ่มต้น) · ขนส่งที่ใช้บริการ (สวิตช์เปิด/ปิด)
- รีวิว & คะแนนร้านค้า (`/seller/reviews`) — สรุปคะแนนร้าน + เกณฑ์ “ร้านแนะนำ” (popup) + แท็บกรองตามดาว · รีวิวรายสินค้า (`/seller/reviews/:id`: ตารางรีวิว + lightbox รูป/วิดีโอ)
- แชท (`/seller/chat`) — รายการแชท + ห้องแชท, ส่งข้อความ/อีโมจิ (emoji picker library)/รูปภาพ (กดขยายได้), การ์ดสินค้าในแชท; ใช้คอมโพเนนต์ร่วม `ChatWorkspace` กับฝั่งลูกค้า
- กระเป๋าเงิน (`/seller/wallet`) — การ์ดสรุป (ยอดถอนได้ / ESCROW / รายได้สะสม / ยอดเงินสะสม) + ประวัติธุรกรรม (กรองด้วย `DateRangePicker`) + modal ถอนเงิน 4 สถานะ (ฟอร์ม → กำลังดำเนินการ → สำเร็จ → ไม่สำเร็จ)
- การตั้งค่าร้านค้า (`/seller/settings`) — 5 แท็บ: จัดการข้อมูลร้านค้า (ข้อมูลเข้าสู่ระบบ / ฟอร์มข้อมูลร้าน / ข้อมูลธุรกิจ-บริษัท-คณะกรรมการแบบอ่านอย่างเดียว) · บัญชีธนาคาร (รายการบัญชีถอนเงิน + การถอนอัตโนมัติ + รหัสผ่าน) · โหมดวันหยุด (ตารางช่วงวันหยุด + `RangeCalendar` + modal เพิ่มวันหยุด) · การแจ้งเตือน (สวิตช์ 4 รายการ) · ปิดบัญชีร้านค้า
- Sidebar ร้านค้า — ปุ่ม “ย่อเมนู” พับ sidebar เป็นโหมดไอคอนล้วน (จำสถานะใน `localStorage`)

**ฝั่งลูกค้า — แชทลอย (FAB)**
- ปุ่มแชทลอยมุมขวาล่าง (`ChatFab`) เปิด/ปิดแผงแชท (`ChatPanel`) — รายการร้านที่คุยด้วย ⇄ ห้องแชท, ส่งข้อความ/อีโมจิ/รูปภาพ, กดขยายรูป; ซ่อนอัตโนมัติในโซน `/seller/*`
...
### ค้างไว้ / ยังไม่ทำ
- หน้า Seller ส่วนที่เหลือ (การวิเคราะห์)
...
| `/seller/products` · `/seller/products/new` | การจัดการสินค้า · ฟอร์มเพิ่ม/แก้ไขสินค้า (stepper 4 ขั้น + variant matrix) |
| `/seller/refunds` · `/seller/refunds/:id` | คำขอคืนเงิน/คืนสินค้า · รายละเอียด (review state machine + timeline + lightbox) |
| `/seller/promotions` · `/seller/discounts` | โปรโมชั่น · โค้ดส่วนลด |
| `/seller/flashsale` · `/seller/flashsale/:id` | Flash Sale (event cards + ตาราง) · รายละเอียด Flash Sale |
| `/seller/logistics` | โลจิสติกส์ — ที่อยู่คลังสินค้า + ตั้งค่าการจัดส่ง + ขนส่งที่ใช้บริการ |
| `/seller/reviews` · `/seller/reviews/:id` | รีวิว & คะแนนร้านค้า · รีวิวรายสินค้า (lightbox) |
| `/seller/chat` | แชท (ร้านค้า) — รายการแชท + ห้องแชท + อีโมจิ/รูปภาพ |
| `/seller/wallet` | กระเป๋าเงิน — สถิติ + ประวัติธุรกรรม (กรองช่วงวันที่) + modal ถอนเงิน 4 สถานะ |
| `/seller/settings` | การตั้งค่าร้านค้า — 5 แท็บ: ข้อมูลร้านค้า / บัญชีธนาคาร / โหมดวันหยุด / การแจ้งเตือน / ปิดบัญชีร้านค้า |
